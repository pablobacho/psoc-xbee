...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_XBEE_UART) || \
         ...
         
{
    ...
    do
    {
        XBEE_UART_PutChar(pData[buf_index]);
        buf_index++;
         ...
         
{
    ...
    *  If atleast one byte is received within the timeout interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != XBEE_UART_GetRxBufferSize())
        {
            ...
            */
            do
            {
                ...
                oldDataCount = XBEE_UART_GetRxBufferSize();
                CyDelay(XBEE_UART_BYTE2BYTE_TIME_OUT);
            }while(XBEE_UART_GetRxBufferSize() > oldDataCount);
    ...
    *count = 0u;
    ...
    while(XBEE_UART_GetRxBufferSize() > 0u)
    {
        tempCount = XBEE_UART_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = XBEE_UART_ReadRxData();
                dataIndexCntr++;
            }
